...
DROP PROCEDURE IF EXISTS view_patient_treatment;
CREATE PROCEDURE view_patient_treatment(
    IN start_date DATE,
    IN end_date DATE,
...
DROP PROCEDURE IF EXISTS view_doctor_work;
CREATE PROCEDURE view_doctor_work(
    IN start_date DATE,
    IN end_date DATE,
    IN email VARCHAR(100),
    OUT result INT,
...
DROP PROCEDURE IF EXISTS view_staff_job_change;
CREATE PROCEDURE view_staff_job_change(
    IN start_date DATE,
    IN end_date DATE,
    IN email VARCHAR(100),
    OUT result INT,
...
BEGIN
	...
	IF email is NULL THEN
		SELECT *
			FROM staff_job_change_report SR
			WHERE SR.created_date BETWEEN start_date AND end_date;
	ELSE
		SELECT *
		FROM staff_job_change_report SR
		WHERE SR.created_date BETWEEN start_date AND end_date
		AND email = SR.staff_email;
	END IF;
